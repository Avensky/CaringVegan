...
const Shop = (props) => {
  const purchaseContinueHandler = async (addedItems, isAuth) => {
  ...
  if (props.addedItems) {
    orderSummary = (
      <OrderSummary
        ...
        purchaseContinued={() =>
          purchaseContinueHandler(props.addedItems, props.isAuth)
        }
      />
    );
  }
  ...
  props.totalItems > 0 ? (checkout = purchaseHandler) : (checkout = null); // eslint-disable-line no-unused-vars
  ...
  let newitem;
  if (props.isAuth) {
    props.isAuth.role === "admin" ? (newitem = <NewItem />) : (newitem = null);
  }
  return (
    <Wrapper>
